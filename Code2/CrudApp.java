...
class CustomerOperations{
	...
	static long totalId(){
		...
		filter(customer->customer.getId()>2000)
		.mapToInt(customer->customer.getId()).sum();
//		long sum =0;
//		for(Customer customer : customers){
//			if(customer.getId()>2000)
//			sum+=customer.getId();
//		}
//		return sum;
	}
	...
	static void sort(){
		//Collections.sort(customers); // trigger Comparable
		//Collections.sort(customers,new SortByName());  // trigger Comparator
		/*Collections.sort(customers,new Comparator<Customer>(){
			public int compare(Customer first, Customer second){
				return first.getCity().compareToIgnoreCase(second.getCity());
			}
		});*/
...
public class CrudApp {
	...
	public static void main(String[] args) {
		...
		while(true){
			...
			case 6:
				System.out.println("Total "+CustomerOperations.getCustomers().size());
				break;
			case 1:
